...
from bert.sentiment_modeling import BertForInteractSpanExtractAndClassification, BertForInteract1, BertForInteract2, \
    BertForInteract3, BertForInteract4
...
from absa.run_base import copy_optimizer_params_to_model, set_optimizer_params_grad, prepare_optimizer, \
...
def run_train_epoch(args, global_step, model, param_optimizer,
    ...
    for step, batch in enumerate(train_dataloader):
        ...
        batch_start_logits, batch_end_logits, batch_aspect_num, _ = model('extract_inference', input_mask,
                                                                          input_ids=input_ids,
                                                                          token_type_ids=segment_ids, logger=logger)
        ...
        span_starts, span_ends, span_aspect_num, labels, label_masks = span_annotate_candidates(train_examples,
                                                                                                batch_features,
                                                                                                batch_results,
...
def evaluate(args, model, device, eval_examples, eval_features, eval_dataloader, logger, write_pred=False):
    ...
    for batch in eval_dataloader:
        ...
        for j, example_index in enumerate(example_indices):
            ...
            batch_results.append(RawSpanResult(unique_id=unique_id, start_logits=start_logits, end_logits=end_logits,
                                               aspect_num_logits=aspect_num_logits))
        span_starts, span_ends, aspect_nums, _, label_masks = span_annotate_candidates(eval_examples, batch_features,
                                                                                       batch_results,
    ...
    metrics, all_nbest_json = eval_absa(eval_examples, eval_features, all_results, args.do_lower_case,
                                        args.verbose_logging, logger)
    if write_pred:
...
def main():
    ...
    ## Required parameters
    # Bert Base
    parser.add_argument("--bert_config_file", default="/data/bert-base-uncased/bert_config.json",
                        type=str,
                        required=False,
                        help="The config json file corresponding to the pre-trained BERT model. "
                             "This specifies the model architecture.")
    parser.add_argument("--vocab_file", default="/data/bert-base-uncased/vocab.txt", type=str,
                        required=False,
                        help="The vocabulary file that the BERT model was trained on.")
    parser.add_argument("--init_checkpoint", default="/data/bert-base-uncased/pytorch_model.bin",
                        type=str,
                        help="Initial checkpoint (usually from a pre-trained BERT model).")
    parser.add_argument("--output_dir", default="/data/out/interact/Lap", type=str,
                        required=False,
    ...
    # parser.add_argument("--train_file", default='rest_total_train.txt', type=str, help="SemEval xml for training")
    # parser.add_argument("--predict_file", default='rest_total_test.txt', type=str, help="SemEval csv for prediction")
    ...
    # parser.add_argument("--train_file", default='twitter1_train.txt', type=str, help="SemEval xml for training")
    # parser.add_argument("--predict_file", default='twitter1_test.txt', type=str, help="SemEval csv for prediction")
    ...
    # parser.add_argument("--dense_hidden_size", type=int, default=300, help="dense hidden size")
    parser.add_argument("--output_hidden_size", type=int, default=50, help="output hidden size")
    parser.add_argument("--warmup_proportion", default=0.1, type=float,
                        help="Proportion of training to perform linear learning rate warmup for. E.g., 0.1 = 10% "
                             "of training.")
    parser.add_argument("--save_proportion", default=0.0, type=float,
    ...
    model = BertForInteractSpanExtractAndClassification(bert_config, args.output_hidden_size)
    if args.init_checkpoint is not None and not os.path.isfile(save_path):
        ...
        logger.info("Loading model mm pretrained checkpoint: {}".format(args.init_checkpoint))
