...
RawSpanCollapsedResult = collections.namedtuple("RawSpanCollapsedResult",
                                                ["unique_id", "neu_start_logits", "neu_end_logits", "pos_start_logits",
                                                 "pos_end_logits",
                                                 "neg_start_logits", "neg_end_logits"])

RawBIOResult = collections.namedtuple("RawBIOResult", ["unique_id", "bio_pred"])

RawBIOClsResult = collections.namedtuple("RawBIOClsResult",
                                         ["unique_id", "start_indexes", "end_indexes", "bio_pred", "span_masks"])
...
RawBIOFinalResult = collections.namedtuple("RawFinalResult",
                                           ["unique_id", "start_indexes", "end_indexes", "ae_logits",
                                            "cls_pred", "span_masks"])

RawHuFinalResult = collections.namedtuple("RawFinalResult",
                                          ["unique_id", "start_indexes", "end_indexes", "start_logits", "end_logits",
                                           "cls_pred", "span_masks"])


def _get_true_indexes(logits, feature):
...
def _get_best_indexes_hu(logits, n_best_size):
    """Get the n-best logits from a list."""
    index_and_score = sorted(enumerate(logits), key=lambda x: x[1], reverse=True)
...
def span_annotate_candidates_crf(all_examples, batch_features, batch_results, filter_type, is_training, use_heuristics,
                                 use_nms,
    ...
    for (feature_index, feature) in enumerate(batch_features):
        ...
        if is_training:
            ...
            is_start = False
            for index, logit in zip(range(len(feature.tokens)), result.bio_pred):
                if index not in feature.token_to_orig_map:
                    continue
                if logit[0] > logit[1] and logit[0] > logit[2]:
                    if is_start:
                        end_index = index - 1
                        while end_index not in feature.token_to_orig_map:
                            end_index -= 1
                    ...
                    if not is_start:
                        ...
                        is_start = True
            if is_start:
                end_index = len(feature.tokens) - 1
                while end_index not in feature.token_to_orig_map:
                    end_index -= 1
                prelim_predictions_per_feature.append(
                    _PrelimPrediction(
...
def span_annotate_candidates_hu(all_examples, batch_features, batch_results, filter_type, is_training, use_heuristics,
                                use_nms,
                                logit_threshold, n_best_size, max_answer_length, do_lower_case, verbose_logging,
                                logger):
    """Annotate top-k candidate answers into features."""
    unique_id_to_result = {}
    for result in batch_results:
    ...
    for (feature_index, feature) in enumerate(batch_features):
        ...
        if is_training:
            ...
            prelim_predictions_per_feature = []
            start_indexes = _get_best_indexes_hu(result.start_logits, n_best_size)
            end_indexes = _get_best_indexes_hu(result.end_logits, n_best_size)
...
def span_annotate_candidates(all_examples, batch_features, batch_results, filter_type, is_training, use_heuristics,
                             use_nms, n_best_size, max_answer_length, do_lower_case, verbose_logging, logger):
    """Annotate top-k candidate answers into features."""
    unique_id_to_result = {}
    for result in batch_results:
        unique_id_to_result[result.unique_id] = result
    ...
    for (feature_index, feature) in enumerate(batch_features):
        ...
        if is_training:
            ...
            total_start_num, real_start_num, _ = _get_true_indexes(result.start_logits, feature)
            total_end_num, real_end_num, _ = _get_true_indexes(result.end_logits, feature)
