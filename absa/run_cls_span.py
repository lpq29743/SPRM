...
def eval_absa_bio(all_examples, all_features, all_results, do_lower_case, verbose_logging, logger):
    unique_id_to_result = {}
...
def eval_absa_hu(all_examples, all_features, all_results, do_lower_case, verbose_logging, logger):
    unique_id_to_result = {}
    for result in all_results:
        unique_id_to_result[result.unique_id] = result
    ...
    for (feature_index, feature) in enumerate(all_features):
        ...
        prediction = {'pred_terms': pred_terms, 'pred_polarities': pred_polarities, 'start_logits': result.start_logits,
...
def eval_absa(all_examples, all_features, all_results, do_lower_case, verbose_logging, logger):
    ...
    for (feature_index, feature) in enumerate(all_features):
        ...
        prediction = {'pred_terms': pred_terms, 'pred_polarities': pred_polarities, 'start_logits': result.start_logits,
                      'end_logits': result.end_logits, "aspect_num": result.aspect_num}
        all_nbest_json[example.example_id] = prediction
...
def main():
    ...
    ## Required parameters
    parser.add_argument("--bert_config_file", default="/data/bert-large-uncased/bert_config.json",
                        type=str,
                        required=False,
                        help="The config json file corresponding to the pre-trained BERT model. "
                             "This specifies the model architecture.")
    parser.add_argument("--vocab_file", default="/data/bert-large-uncased/vocab.txt", type=str,
                        required=False,
                        help="The vocabulary file that the BERT model was trained on.")
    parser.add_argument("--init_checkpoint", default="/data/bert-large-uncased/pytorch_model.bin",
                        type=str,
                        help="Initial checkpoint (usually from a pre-trained BERT model).")
    parser.add_argument("--output_dir", default="/data/out/extract/02", type=str, required=False,
    ...
    parser.add_argument("--do_pipeline", default=False, action='store_true',
