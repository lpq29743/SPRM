...
def get_span_representation(span_starts, span_ends, input, input_mask):
    ...
    span_indices = torch.arange(JR).unsqueeze(0).to(span_starts_offset.device) + span_starts_offset.unsqueeze(
        1)  # [N*M, JR]
...
def distant_cross_entropy_hu(logits, positions, mask=None):
    '''
    :param logits: [N, L]
    :param positions: [N, L]
    :param mask: [N]
...
def distant_cross_entropy(logits, positions, mask=None):
    ...
    aspect_num = torch.sum((sigmoid(logits) > 0.5).to(dtype=logits.dtype) * mask, -1)
...
class BertForSpanAspectClassification(nn.Module):
    ...
    def forward(self, mode, attention_mask, input_ids=None, token_type_ids=None, span_starts=None, span_ends=None,
        ...
        if mode == 'train':
            ...
            mean_loss = torch.sum(loss * flat_label_masks.to(dtype=loss.dtype)) / torch.sum(
                flat_label_masks.to(dtype=loss.dtype))
            return mean_loss
...
class BertForJointSpanExtractAndClassification(nn.Module):
    ...
    def forward(self, mode, attention_mask, input_ids=None, token_type_ids=None, start_positions=None,
                end_positions=None, span_starts=None, span_ends=None, polarity_labels=None, label_masks=None,
                sequence_input=None):
        if mode == 'train':
            assert input_ids is not None and token_type_ids is not None
            all_encoder_layers, _ = self.bert(input_ids, token_type_ids, attention_mask)
...
class BertForInteractBIOExtractAndClassification(nn.Module):
    def __init__(self, config, output_hidden_size):
        super(BertForInteractBIOExtractAndClassification, self).__init__()
        self.bert = BertModel(config)
    ...
    def forward(self, mode, attention_mask, input_ids=None, token_type_ids=None, bio_labels=None,
                polarity_positions=None, span_starts=None, span_ends=None, polarity_labels=None, label_masks=None,
                sequence_input=None, device=None, logger=None):
        if mode == 'train':
            assert input_ids is not None and token_type_ids is not None
            all_encoder_layers, _ = self.bert(input_ids, token_type_ids, attention_mask)
            bert_sequence_output = all_encoder_layers[-1]
            ...
            ae_logits = self.bio_affine(ae_final_sequence_output)
            ac_final_sequence_output = torch.cat([bert_sequence_output, ac_sequence_output], -1)
            return ae_logits, ac_final_sequence_output
...
class BertForInteractHuExtractAndClassification(nn.Module):
    def __init__(self, config, output_hidden_size):
        super(BertForInteractHuExtractAndClassification, self).__init__()
        self.bert = BertModel(config)
    ...
    def forward(self, mode, attention_mask, input_ids=None, token_type_ids=None, start_positions=None,
                end_positions=None, span_starts=None, span_ends=None, polarity_labels=None, label_masks=None,
                sequence_input=None, logger=None):
        if mode == 'train':
            assert input_ids is not None and token_type_ids is not None
            all_encoder_layers, _ = self.bert(input_ids, token_type_ids, attention_mask)
            bert_sequence_output = all_encoder_layers[-1]
            # bert_sequence_output = self.bert_dense(bert_sequence_output)
            ...
            # ae_sequence_output = self.ae_cnn(sentence_inputs.unsqueeze(1))
            # ae_sequence_output = torch.relu(ae_sequence_output)
            # ae_sequence_output = self.ae_dense(ae_sequence_output.squeeze(-1).permute(0, 2, 1))
            # ac_sequence_output = self.ac_cnn(sentence_inputs.unsqueeze(1))
            # ac_sequence_output = torch.relu(ac_sequence_output)
            # ac_sequence_output = self.ac_dense(ac_sequence_output.squeeze(-1).permute(0, 2, 1))

            assert start_positions is not None and end_positions is not None
            # Aspect Extraction Prediction

            # w/o Shared Part
            # ae_final_sequence_output = ae_sequence_output
            # w/o Private Part
            # ae_final_sequence_output = bert_sequence_output
            # Final
            ae_final_sequence_output = torch.cat([bert_sequence_output, ae_sequence_output], -1)
            ...
            start_loss = distant_cross_entropy_hu(start_logits, start_positions)
            end_loss = distant_cross_entropy_hu(end_logits, end_positions)
            ae_loss = (start_loss + end_loss) / 2
            ...
            end_logits = end_logits.squeeze(-1)
            ac_final_sequence_output = torch.cat([bert_sequence_output, ac_sequence_output], -1)
            return start_logits, end_logits, ac_final_sequence_output
...
class BertForInteractSpanExtractAndClassification(nn.Module):
    def __init__(self, config, output_hidden_size):
        ...
        self.ac_bilstm = nn.LSTM(input_size=config.hidden_size, hidden_size=config.hidden_size, batch_first=True,
                                 bidirectional=True)
        # M2: CNN
        # self.ae_cnn = nn.Conv2d(in_channels=1, out_channels=dense_hidden_size * 2, kernel_size=(5, config.hidden_size),
        #                         padding=(2, 0))
        # self.ac_cnn = nn.Conv2d(in_channels=1, out_channels=dense_hidden_size * 2, kernel_size=(5, config.hidden_size),
        #                         padding=(2, 0))

        # self.bert_dense = nn.Linear(config.hidden_size, dense_hidden_size)
        self.ae_dense = nn.Linear(config.hidden_size * 2, config.hidden_size)
        self.ac_dense = nn.Linear(config.hidden_size * 2, config.hidden_size)

        # Ablation Study
        # self.attention = nn.Linear(config.hidden_size, 1)
        # self.ac_output_layer = nn.Linear(config.hidden_size, output_hidden_size)
        # Final
        self.attention = nn.Linear(config.hidden_size * 2, 1)
        self.ac_output_layer = nn.Linear(config.hidden_size * 2, output_hidden_size)

        # Ablation Study
        # self.extraction = nn.Linear(config.hidden_size, 2)
        # Final
        self.extraction = nn.Linear(config.hidden_size * 2, 2)
    ...
    def forward(self, mode, attention_mask, input_ids=None, token_type_ids=None, start_positions=None,
        ...
        if mode == 'train':
            ...
            # w/o Shared Part
            # ac_final_sequence_output = ac_sequence_output
            # w/o Private Part
            # ac_final_sequence_output = bert_sequence_output
            # Final
            ac_final_sequence_output = torch.cat([bert_sequence_output, ac_sequence_output], -1)
...
class BertForInteract1(nn.Module):
    def __init__(self, config, output_hidden_size):
        super(BertForInteract1, self).__init__()
        # Shared Part
        self.bert = BertModel(config)

        # Private Part
        # M1: LSTM
        self.bilstm = nn.LSTM(input_size=config.hidden_size, hidden_size=config.hidden_size, batch_first=True,
                              bidirectional=True)
        self.ac_bilstm = nn.LSTM(input_size=config.hidden_size, hidden_size=config.hidden_size, batch_first=True,
                                 bidirectional=True)
        # M2: CNN
    ...
    def forward(self, mode, attention_mask, input_ids=None, token_type_ids=None, start_positions=None,
        ...
        if mode == 'train':
            ...
            # Shared Part
            sentence_inputs = self.bert.embeddings(input_ids)
            bert_sequence_output, _ = self.bilstm(sentence_inputs)
            bert_sequence_output = self.dense(bert_sequence_output)

            # bert_sequence_output = self.bert_dense(bert_sequence_output)

            # Private Part
            all_encoder_layers, _ = self.bert(input_ids, token_type_ids, attention_mask)
            ae_sequence_output = all_encoder_layers[-1]
            ac_sequence_output, _ = self.ac_bilstm(sentence_inputs)
            ac_sequence_output = self.ac_dense(ac_sequence_output)
...
class BertForInteract2(nn.Module):
    def __init__(self, config, output_hidden_size):
        super(BertForInteract2, self).__init__()
        # Shared Part
        self.bert = BertModel(config)
    ...
    def forward(self, mode, attention_mask, input_ids=None, token_type_ids=None, start_positions=None,
        ...
        if mode == 'train':
            ...
            # Private Part
            all_encoder_layers, _ = self.bert(input_ids, token_type_ids, attention_mask)
            ac_sequence_output = all_encoder_layers[-1]
            ae_sequence_output, _ = self.ae_bilstm(sentence_inputs)
            ae_sequence_output = self.ae_dense(ae_sequence_output)
...
class BertForInteract3(nn.Module):
    def __init__(self, config, output_hidden_size):
        super(BertForInteract3, self).__init__()
        # Shared Part
        self.bert = BertModel(config)
        ...
        #                          bidirectional=True)
        # M2: CNN
        self.ae_cnn = nn.Conv2d(in_channels=1, out_channels=config.hidden_size, kernel_size=(5, config.hidden_size),
                                padding=(2, 0))
        self.ac_cnn = nn.Conv2d(in_channels=1, out_channels=config.hidden_size, kernel_size=(5, config.hidden_size),
                                padding=(2, 0))

        # self.bert_dense = nn.Linear(config.hidden_size, dense_hidden_size)
        self.ae_dense = nn.Linear(config.hidden_size, config.hidden_size)
        self.ac_dense = nn.Linear(config.hidden_size, config.hidden_size)
...
class BertForInteract4(nn.Module):
    def __init__(self, config, output_hidden_size):
        super(BertForInteract4, self).__init__()
        # Shared Part
        # self.bert = BertModel(config)
        ...
        self.bert_dense = nn.Linear(config.hidden_size * 2, config.hidden_size)
        self.ae_dense = nn.Linear(config.hidden_size * 2, config.hidden_size)
        self.ac_dense = nn.Linear(config.hidden_size * 2, config.hidden_size)
